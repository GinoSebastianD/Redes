...
int main(void){
    ...
    memset(&estructura,0,sizeof(struct sockaddr_in));

    estructura.sin_family = AF_INET;
    estructura.sin_port = htons(1100);
    estructura.sin_addr.s_addr = INADDR_ANY;

    if(-1 == bind(SocketFD, (const struct sockaddr *)&estructura , sizeof(struct sockaddr_in) )){
